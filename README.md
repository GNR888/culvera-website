...
- **Research & Insights** (`research-insights/`) — article library (newest
...
js/
  ...
  footer.js               Renders the footer into #site-footer on every page
                          (links resolve from the site root, so it works on
                          nested pages too)
  footer-data.js          All editable footer copy/links
  research-filters.js     Topic filter chips on the Research & Insights page
...
## Research & Insights: adding an article

Article pages are static HTML generated by a small, dependency-free Node
script, so the site itself still has no build step: run the script, then
commit its output with everything else.

1. Write the body in `content/research-insights/<slug>.html`, starting at
   `<h2>` (the page supplies the `<h1>`, image, sources and related
   articles). Use `{{root}}about.html` for links to site pages and
   `{{article:other-slug}}` for links to other articles.
2. Add an entry to `content/research-insights/articles.mjs` (title, meta
   description, excerpt, topics, date, image + alt text, sources). The
   field list is documented at the top of that file.
3. Run `node scripts/build-research.mjs`. It validates the content and
   regenerates `research-insights/` (never edit those files by hand).

The article appears on the library page automatically, newest first, and
related articles are picked by shared topics. Each page gets its own title,
meta description, canonical link, Open Graph tags and Article schema.

**Set the live domain:** put the site's public URL in `content/site.mjs`
and re-run the script. Until then canonical links are root-relative and
the absolute-URL tags (`og:url`, `og:image`) and `sitemap.xml` are skipped.
